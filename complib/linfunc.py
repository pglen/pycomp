...
def reduce_const(self2, filter, xstack):

    if pvg.opt_debug > 6:
        print("\npaStack pre reduce:", pp(filter), end = " ")
        for aa in paStack:
            print(self2.arrx[aa], end = " ")
        print()

    # Walk the stack
    loopx = 0 ; statex = 0 ; numidx = -1 ; opidx = -1 ;  wasop = False
    while True:
        if loopx >= len(xstack):
            break
        ...
        if self2.arrx[idx].flag != 0:
            loopx += 1
            continue
        #print("pastack: [", self2.arrx[idx].stamp.xstr,
